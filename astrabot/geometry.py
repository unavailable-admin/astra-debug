...
DEFAULT_CALIBRATION = CONFIG / "g1_head_camera_calibrations.yaml"
...
class StereoGeometry:
    ...
    def load(cls, mode, path=DEFAULT_CALIBRATION):
        ...
        for side in ("left", "right"):
            ...
            if mode == "sim":
                ...
                K = np.array([[K[0, 0], 0, size[0] / 2], [0, K[0, 0], size[1] / 2], [0, 0, 1.0]])
                D = np.zeros(5)
    ...
    def triangulate(self, left_px, right_px, *, image_size_wh, max_reprojection_px=2.0):
        ...
        for points, observed, K, D in (
            (xyz, left, self.K_left, self.D_left),
...
def validate_status(status, geometry):
    if status["scene_id"] != "showroom_scene_11_stereo" or not status.get("stereo_enabled"):
    ...
    relative = np.linalg.inv(mounts[1]) @ mounts[0]
    for a, b in (
        (mounts[0], geometry.T_head_left_cv),
        (relative[:3, :3], geometry.R),
        (relative[:3, 3], geometry.T),
    ):
