...
class OfficialOpenAI:
    def __init__(self):
        ...
        self.opener=urllib.request.build_opener(ExplicitProxy({'https':self.proxy}) if self.proxy else urllib.request.ProxyHandler({}),
                                              urllib.request.HTTPSHandler(context=context))

    def metadata(self):
        return {'provider':'openai_official','model':self.model,'base_url':self.base_url,
                'proxy':self.proxy,'key_file':str(self.key_file),'ca_bundle':self.ca_bundle}

    def chat(self,body,timeout=180):
        key=self.key_file.read_text().strip()
        if not key:raise ValueError('Official API key file is empty')
        payload={**body,'model':self.model,'store':False}
        if self.model.startswith(('gpt-4.','gpt-4o')):payload.pop('reasoning_effort',None)
