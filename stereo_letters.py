...
class StereoTracker:
    def __init__(self, sim, cube_size=.04):
        self.sim = sim
        ...
        self.maps = [cv2.initUndistortRectifyMap(K, D, R, P, g.image_size_wh, cv2.CV_32FC1)
                     for K, D, R, P in ((g.K_left,g.D_left,self.R1,self.P1),
    ...
    def locate(self, image, identified=None):
        ...
        if left is None or right is None or left.shape != (480,640,3) or right.shape != left.shape:
    ...
    def locate_images(self,left,right,identified,T):
        ...
        def matcher(min_disp):
            return cv2.StereoSGBM_create(minDisparity=min_disp,numDisparities=128,blockSize=5,
                P1=8*25,P2=32*25,disp12MaxDiff=1,uniquenessRatio=12,
                speckleWindowSize=40,speckleRange=1,mode=cv2.STEREO_SGBM_MODE_SGBM_3WAY)
        ...
        consistent=(disparity>1)&(rx>=0)&(rx<640)&(np.abs(disparity+sampled)<.75)
        ...
        for letter,pixel in identified.items():
            try:
                ...
                points=world[good]
                if len(points)<10:raise ValueError('Fewer than 10 left/right-consistent glyph pixels')
                z=float(np.median(points[:,2]))
                keep=np.abs(points[:,2]-z)<.008
                matched_yx=np.column_stack(np.where(good))[keep]
                points=points[keep]
                if len(points)<10 or keep.mean()<.7:
                    raise ValueError('Glyph depth is not a consistent horizontal surface')
                z=float(np.median(points[:,2]))
                spread=float(np.quantile(np.abs(points[:,2]-z),.9))
                if spread>.006:raise ValueError('Glyph depth spread exceeds 6mm')
                # Use bounding-box center rather than the ink's asymmetric centroid.
                x,y,w,h,_=stats[idx]; u,v=x+w/2,y+h/2
                ray=T[:3,:3]@np.linalg.solve(g.K_left,[u,v,1.])
                distance=(z-T[2,3])/ray[2]
                if distance<=0:raise ValueError('Surface intersection behind camera')
                ...
                correspondences=[{'left_rectified_px':[int(px),int(py)],
                    'right_rectified_px':[float(px-disparity[py,px]),int(py)],
                    'lr_error_px':float(abs(disparity[py,px]+sampled[py,px]))}
                    for py,px in sample]
                letters[letter]={'estimated_xyz':center.tolist(),'top_center_xyz':top.tolist(),
                    'grasp_center_xyz':grasp.tolist(),'pixel':[float(u),float(v)],
                    'valid_depth_pixels':len(points),'depth_p90_spread_m':spread,
                    'source':'stereo_sgbm_lr_checked_glyph_surface','cube_size_m':self.cube_size,
                    'correspondence_samples':correspondences}
            except ValueError as exc:errors[letter]=str(exc)
        if letters:
            tops=np.array([v['top_center_xyz'][2] for v in letters.values()])
            table_z=float(np.median(tops)-self.cube_size)
            if np.ptp(tops)>.015:
                return {'ok':False,'letters':{},'errors':errors,'reason':'Upright cube top heights disagree by >15mm'}
        else:table_z=None
        return {'ok':bool(letters),'letters':letters,'errors':errors,'table_z':table_z,
