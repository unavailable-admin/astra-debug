...
from openai_connection import OfficialOpenAI,ExplicitProxy

class OfficialConnection(unittest.TestCase):
    def test_company_environment_cannot_select_company_key_or_endpoint(self):
        with patch.dict(os.environ,{'ASTRA_API_BASE_URL':'http://company.invalid/v1','ASTRA_MODEL':'old'},clear=True):
    ...
    def test_chat_loads_selected_file_without_storing_key_in_metadata(self):
        with tempfile.TemporaryDirectory() as directory:
            path=Path(directory)/'test_key';path.write_text('dummy-test-key')
            with patch.dict(os.environ,{'OPENAI_API_KEY_FILE':str(path)},clear=True):client=OfficialOpenAI()
