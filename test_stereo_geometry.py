...
from stereo_geometry import DEFAULT_CALIBRATION, StereoGeometry
...
class StereoGeometryTests(unittest.TestCase):
    def test_profiles_and_relative_mounts(self):
        ...
        np.testing.assert_allclose(sim.R, real.R, atol=1e-9)
        np.testing.assert_allclose(sim.T, real.T, atol=1e-9)
        ...
        # Forward optical direction should face forward/down in head axes.
    ...
    def test_roundtrip_raw_pixels_with_and_without_distortion(self):
        xyz = np.array([[-.12, -.10, .6], [.1, .15, .9], [.02, .03, 1.3]])
